...
from robosuite.models.arenas import TargetArena
...
class Push(RobotEnv):
    ...
    def _check_success(self):
        """
        Check if cube is at the target.

        Returns:
            bool: True if cube is within the distance threshold of the target.
        """
        cube_pos = self.sim.data.body_xpos[self.cube_body_id]
        goal_pos = self.sim.data.site_xpos[self.sim.model.site_name2id('target')]
        dist = np.linalg.norm(cube_pos - goal_pos)
