...
class Lift(RobotEnv):
    ...
    def _load_model(self):
        ...
        self.model = TableTopTask( self.mujoco_arena, [robot.robot_model for robot in self.robots], self.mujoco_objects, initializer=self.placement_initializer)
    ...
    def _visualization(self):
        ...
        if self.robots[0].gripper_visualization:
            ...
            dist = np.sum(np.square(self.sim.data.site_xpos[cube_site_id] - self.sim.data.get_site_xpos(self.robots[0].gripper.visualization_sites["grip_site"])))
