...
class DoorOpen(RobotEnv):
	...
	def _check_gripper_contact(self):
		...
		for contact in self.sim.data.contact[:self.sim.data.ncon]:
			if self.sim.model.geom_id2name(contact.geom1) in self.robots[0].gripper.contact_geoms or self.sim.model.geom_id2name(contact.geom2) in self.robots[0].gripper.contact_geoms:
				collision = True
				break
		return collision
