...
from ...utils.mjcf_utils import xml_path_completion, array_to_string, string_to_array
from ..objects import CylinderObject, PlateWithHoleObject, BoxObject
from . import Arena
...
class WipeForceTableArena(Arena):
    ...
    def configure_location(self):
        ...
        if self.draw_line: 
            ...
            collision_c.set("pos", array_to_string([table_cte_size_x/2 - self.table_half_size[0] - 0.18, 0, self.table_half_size[2]+squares_height/2]))
            table_subtree.append(collision_c)
            pos = np.array((np.random.uniform(-self.table_half_size[0], self.table_half_size[0]), np.random.uniform(-self.table_half_size[1], self.table_half_size[1])))
            direction =  np.random.uniform(-np.pi, np.pi)

            if not self.two_clusters:
                for i in range(self.num_sensors):
                    square2 = CylinderObject(size=[self.line_width, 0.001], rgba=[0, 1,0, 0], density=1, friction=friction)
                    ...
                    collision_c.find("site").set("pos", array_to_string([0,0,0.005]))
            ...
            for i in range(self.num_squares[0]):
                for j in range(self.num_squares[1]):
                    ...
                    squares_separation = 0.0
                    squares_height = 0.1
                    square2 = BoxObject(size=[self.square_half_size[0]-0.0005, self.square_half_size[1]-squares_separation, squares_height/2 - 0.001], rgba=[0, 1, 0, 1], density=1, friction=0.0001)
                    square_name2 = 'contact_'+str(i) + "_" + str(j)
                    self.squares[square_name2] = square2
                    collision_c = square2.get_collision(name=square_name2, site=True)
                    collision_c.set("pos", array_to_string([self.table_half_size[0]-i*self.square_full_size[0]-0.5*self.square_full_size[0], self.table_half_size[1]-j*self.square_full_size[1]-0.5*self.square_full_size[1], self.table_half_size[2]+squares_height/2 - 0.001]))
            ...
            square2 = BoxObject(size=[border_half_size, self.table_half_size[1] + 2*border_half_size, squares_height/2 - 0.001], rgba=[0, 1, 0, 1], density=1, friction=0.05)
            ...
            square2 = BoxObject(size=[self.table_half_size[0], border_half_size, squares_height/2 - 0.001], rgba=[0, 1, 0, 1], density=1, friction=0.05)
            ...
            square2 = BoxObject(size=[low_border_half_size, self.table_half_size[1] + 2*border_half_size, squares_height/2 - 0.001], rgba=[0, 1, 0, 1], density=1, friction=0.05)
